    ...
    <div>
        <div class="max-w-4xl mx-auto py-10 sm:px-6 lg:px-8">
            ...
            <div class="mt-5 md:mt-0 md:col-span-2">
                <form method="post" action="{{ route('lists_control.update', $list) }}">
                    @csrf
                    @method('PUT')
                    <div class="shadow overflow-hidden sm:rounded-md">
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <input type="text" name="{{$form_elem}}" id="{{$form_elem}}" value="{{ old($list->{$form_elem}, $list->{$form_elem}) }}" class="form-input rounded-md shadow-sm mt-1 block w-full" placeholder="short" />
                            <small id="{{$form_elem}}Help" class="block font-medium text-sm text-gray-500">It'll be shown as title when the list is used.</small>
                            @error($form_elem)
