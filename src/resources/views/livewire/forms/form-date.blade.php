<div>
    ...
    @if($rangeable)
        <div class="row">
            <div class="col-4">
                ...
                <input value="{{ (old($form_elem, $previous ?? ""))['min'] ?? ""}}" type="date" name="{{ $form_elem }}[min]" id="{{ $form_elem }}-min"
    ...
    @if($rangeable)
            ...
            <div class="col-4">
                ...
                <input value="{{ (old($form_elem, $previous ?? ""))['max'] ?? "" }}" type="date" name="{{ $form_elem }}[max]" id="{{ $form_elem }}-max"
                       class="form-input rounded-md shadow-sm mt-1 block w-full"
                       placeholder="{{ $placeHolder }}"/>
            </div>
        </div>
