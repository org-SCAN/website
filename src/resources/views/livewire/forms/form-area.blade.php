<div>
    @foreach($divs as $index => $div)
        <div wire:key="div-{{ $index }}" class="mt-4">
            ...
            @for($i = 0; $i < 4; $i++)
                <div class="flex">
                    <div class="flex-auto pr-4">
                        ...
                        <input type="text" name="{{ $form_elem }}[listCoordinates][{{ $i }}][lat]" id="lat{{ $i }}"
                    ...
                    <div class="flex-auto pr-4">
                        ...
                        <input type="text" name="{{ $form_elem }}[listCoordinates][{{ $i }}][long]" id="long{{ $i }}"
                               class="form-input rounded-md shadow-sm mt-1 block w-full"
                               value="{{(old($form_elem, $previous ?? ""))['lat'] ?? ""}}"
                               placeholder="{{ $placeHolder }}"/>
                    </div>
